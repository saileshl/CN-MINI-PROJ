...
class NetworkAgent:
    ...
    async def connect(self):
        ...
        if self.credentials:
            ...
            self.pairing_code = input("[?] Enter pairing code from website: ").strip().upper()
            url = f"{self.config['backend_url']}?code={self.pairing_code}"
            print(f"[*] Pairing with code: {self.pairing_code}")
        ...
        try:
            ...
            if auth_data.get("type") == "auth_error":
                ...
                if self.credentials:
                    print("[*] Stored credential may be revoked. Use --reset to re-pair.")
                return False
            ...
            if auth_data.get("type") == "auth_success":
                ...
                # Save credentials if this was a first-time pairing
                if agent_token:
    ...
    async def handle_command(self, data):
        ...
        if cmd_type == "start_test":
            ...
            print("[*] Test stopped by dashboard")
    ...
    async def run_test(self):
        """Run a single ad-hoc UDP test."""
        print(f"\n[*] Starting UDP test ({self.config['packet_count']} packets)...")

        buffer = self.jitter_buffer if self.mitigation_enabled else None
        if buffer:
            buffer.reset()

        results, metrics = run_udp_test(self.config, buffer)

        # Stream batch results
        batch_size = 10
        for i in range(0, len(results), batch_size):
            batch = results[i:i + batch_size]
            rtt_batch = [r["rtt"] for r in batch if r.get("rtt") is not None]
            ...
            await self.ws.send(json.dumps({
                ...
                "progress": min(i + batch_size, len(results)) / len(results),
        ...
        print(f"    Avg RTT: {metrics.get('avg_rtt', 'N/A')} ms")
        print(f"    RTT Variation: {metrics.get('avg_rtt_variation', 'N/A')} ms")
        print(f"    Packet Loss: {metrics.get('packet_loss_percent', 'N/A')}%")
        if buffer:
            bs = metrics.get("buffer_stats", {})
            print(f"    Buffer Depth: {bs.get('target_depth_ms', 'N/A')} ms")
            print(f"    Effective Delivery Var: {bs.get('effective_delivery_variation', 'N/A')} ms")
            print(f"    Dropped (late): {bs.get('packets_dropped_late', 0)}")
    ...
    async def run_experiment(self, data):
        ...
        print(f"    [A] Avg RTT: {metrics_a.get('avg_rtt', 'N/A')} ms")
        print(f"    [A] RTT Variation: {metrics_a.get('avg_rtt_variation', 'N/A')} ms")
        print(f"    [A] Packet Loss: {metrics_a.get('packet_loss_percent', 'N/A')}%")
        ...
        results_b, metrics_b = run_udp_test(test_config, jitter_buffer=buffer)
        ...
        print(f"    [B] Avg RTT: {metrics_b.get('avg_rtt', 'N/A')} ms")
        print(f"    [B] RTT Variation (raw): {metrics_b.get('avg_rtt_variation', 'N/A')} ms")
        bs = metrics_b.get("buffer_stats", {})
        print(f"    [B] Effective Delivery Var: {bs.get('effective_delivery_variation', 'N/A')} ms")
        print(f"    [B] Buffer Depth: {bs.get('target_depth_ms', 'N/A')} ms")
        print(f"    [B] Dropped (late): {bs.get('packets_dropped_late', 0)}")
...
Reset credentials:
  python network_agent.py --reset
    ...
    def signal_handler(sig, frame):
        ...
        agent.running = False
        if agent.ws:
            asyncio.get_event_loop().run_until_complete(agent.ws.close())
        sys.exit(0)
